...
class ModelOutput:
    ...
    ubtime: np.ndarray
    psitime: np.ndarray
    epdivtime: np.ndarray  # EP flux divergence (∂F/∂z); model convention: epdiv = -Cu
    psi0_final: float
...
def run_model(s: int = 2, hb: float = 200.0, days: int = 90, dt: float = 3600.0) -> ModelOutput:
    ...
    epdivtime = np.zeros((nl, days))  # daily EP flux divergence (∂F/∂z)
    ...
    tsec = 0.0
    psi0 = 0.0
    ep_div_current = np.zeros(n, dtype=np.float64)
    ...
    for istep in range(ntime):
        ...
        # EP flux divergence (∂F/∂z) is the negative of the convergence term Cu
        ep_div_current = -Cu
        ...
        if (istep + 1) % steps_per_day == 0:
            ...
            # Put 0 at the 16 km boundary point (EP term is computed on prediction levels)
            epdivtime[:, day_idx] = np.concatenate(([0.0], ep_div_current))
...
def _plot(out: ModelOutput) -> None:
    ...
    plt.figure()
    plt.pcolormesh(D, Z, out.epdivtime, shading="auto")
    plt.colorbar(label=r"m/s$^2$")
    plt.xlabel("time (days)")
    plt.ylabel("height (km)")
    plt.title("EP flux divergence (∂F/∂z)")
    plt.show()
